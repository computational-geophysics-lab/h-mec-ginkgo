...
const Eigen::VectorXd x = Eigen::VectorXd::LinSpaced(Nx, xbeg, xend); // horizontal coordinates of basic grid points
const Eigen::VectorXd y = Eigen::VectorXd::LinSpaced(Ny, ybeg, yend); // vertical coordinates of basic grid points
const Eigen::VectorXd xvx = Eigen::VectorXd::LinSpaced(Nx1, xbeg, xend + dx); // Horizontal coordinates of Vx - nodes
const Eigen::VectorXd yvx = Eigen::VectorXd::LinSpaced(Ny1, ybeg - dy / 2., yend + dy / 2.); // Vertical coordinates of Vx - nodes
const Eigen::VectorXd xvy = Eigen::VectorXd::LinSpaced(Nx1, xbeg - dx / 2., xend + dx / 2.); // Horizontal coordinates of Vy - nodes
const Eigen::VectorXd yvy = Eigen::VectorXd::LinSpaced(Ny1, ybeg, yend + dy); // Vertical coordinates of Vy - nodes
const Eigen::VectorXd xp = Eigen::VectorXd::LinSpaced(Nx1, xbeg - dx / 2., xend + dx / 2.); // Horizontal coordinates of P - nodes
const Eigen::VectorXd yp = Eigen::VectorXd::LinSpaced(Ny1, ybeg - dy / 2., yend + dy / 2.); // Vertical coordinates of Vx - nodes
...
//                      Block  Fault
const Eigen::Vector2d arsfm = {.025, .006  }; // a - parameter of RSF
const Eigen::Vector2d brsfm = {.001, .015  }; // b - parameter of RSF
const Eigen::Vector2d lrsfm = {.020, .0085 }; // L - parameter of RSF (characteristic slip distance)
const Eigen::Vector2d omm   = {10,   -10   }; // State
...
const Eigen::VectorXd gm = Eigen::VectorXd::Constant(marknum, shearmod); // Standard shear modulus of bulk, Pa
const Eigen::VectorXd rhofm = Eigen::VectorXd::Constant(marknum, 1000); // Density of fluid
const Eigen::VectorXd etafm = Eigen::VectorXd::Constant(marknum, 1e-3); // Viscosity of fluid    
